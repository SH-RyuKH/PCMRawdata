...
class ExcelChartApp:
    ...
    def create_widgets(self):
        ...
        # 삭제 버튼 생성
        self.delete_button = tk.Button(self.root, text="Delete Selected Data", command=self.delete_data)
        ...
        # 데이터 다운로드 버튼 생성
        self.download_button = tk.Button(self.root, text="Download Data as Excel", command=self.download_data)
    ...
    def load_data(self):
        ...
        if file_path:
            try:
                ...
                new_column = []
                current_group = 0
                for slot in self.data['Slot']:
                    if slot == 1:
                        current_group += 1
                    new_column.append(current_group)

                self.data['New_Column'] = new_column
    ...
    def on_click(self, event):
        if event.xdata is not None and event.ydata is not None:
            ...
            self.delete_button.config(text="Delete Selected Data", command=self.delete_data)
            ...
            self.selected_y_value_textbox.delete(0, tk.END)
            self.selected_y_value_textbox.config(state="readonly")
            self.delete_button.config(text="Plot Chart", command=self.plot_selected_column)
    ...
    def delete_data(self):
        ...
        if selected_column_index:
            ...
            if not selected_column:
                messagebox.showerror("Error", "Please select a column to delete data from.")
                return
            ...
            try:
                if self.selected_index is not None:
                    ...
                    # 변경된 데이터로 차트 업데이트
                    self.ax.clear()
                    self.ax.plot(self.data[selected_column].tolist(), 'o')  # 'o' 스타일로 점만 표시
                    self.ax.set_xlabel('Data Index')
                    self.ax.set_ylabel('Data Value')
                    self.ax.set_title(f'{selected_column} Chart')
                    self.canvas.draw()
                    ...
                    self.delete_button.config(text="Select Data", command=self.plot_selected_column)

                else:
                    messagebox.showerror("Error", "No data selected. Please select data to delete.")
